...
SOURCES = [{"title": "PruebaRegio", "url": "http://srregio.net/addonregio/addon.xml", "icon": "https://srregio.net/notiregio/wp-content/uploads/2021/10/00regioplaylogo-512x475.png", "fanart": "http://bgapp.live/black/img/fanart.jpg",}]
	
def convert2str(text=''):
    codecs = ['utf8', 'cp1252', 'latin-1', 'iso-8859-1', 'ascii']
...
def addon_log(string, level=xbmc.LOGDEBUG):
    if debug == 'true':
        xbmc.log("[plugin.video.blackghost-{0}]: {1}".format(addon_version, string), LOGINFO)
    else:
        xbmc.log("[plugin.video.blackghost-{0}]: {1}".format(addon_version, string), level)
