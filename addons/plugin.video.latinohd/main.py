...
def LatinoHd(szuk = False):
	...
	html = request_sess('https://www.cablevisionhd.com/', 'get', headers=headers)
...
def PlayLatino(url):
	...
	try:
		...
		if 'url' in pa2 and 'kid' in pa2:
			...
			headers = {
				...
				'Origin': 'https://embed.cablevisionhd.com',
				'DNT': '1',
	...
	#if ('livestream' in url or '/stream/' in url) and 'php' in url:
	if 'livestream' in url and 'php' in url:
		host = urlparse(url).netloc
		headers = {
			'Host': host,#'live.cablevisionhd.com',
			'User-Agent': UAx,
			'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8',
			'Accept-Language': 'pl,en-US;q=0.7,en;q=0.3',
			'Referer': 'https://www.cablevisionhd.com/',
		...
		nturl3 = 'https://'+host#live.cablevisionhd.com'
	...
	if stream_url:
		headers.update({'referer': 'https://'+host})#ive.cablevisionhd.com/'})
